...
function createFlower({ x, stemH, head, delay, sway, speed, petals = 12, stemX = x }) {
...
function populateGarden() {
  ...
  bouquet.forEach((pos, i) => {
    ...
    const delay = i * 0.10 + Math.random() * 0.18;
    const sway = 2.2 + Math.random() * 1.3;
    const speed = 4.2 + Math.random() * 1.2;
    const petals = 11 + Math.floor(Math.random() * 2);

    const flower = createFlower({ x: pos.x, stemH, head, delay, sway, speed, petals });
